...
#include "map_generator/moving_cylinder_real.h"
...
void RandomMapGenerate() {
  ...
  for (int i = 0; i < _obs_num; i++) {
    if(i==0){
      obs_x = obs1_pose.pose.position.x;
      obs_y = obs1_pose.pose.position.y;
    }
    if(i==1){
      obs_x = obs2_pose.pose.position.x;
      obs_y = obs2_pose.pose.position.y;
    }
    if(i==2){
      obs_x = obs3_pose.pose.position.x;
      obs_y = obs3_pose.pose.position.y;
      ROS_INFO("obs3 generated");
    }
    dynamic_map_objects::MovingCylinder cylinder(_x_l, _x_h, _y_l, _y_h, _w_l, _w_h, _h_l, _h_h,
                                                 _v_h, eng, _resolution, obs_x, obs_y);
...
void obs1_pose_set_callback(const geometry_msgs::PoseStampedConstPtr & msg_ptr){
    obs1_pose.pose.position.x = msg_ptr->pose.position.x;
    obs1_pose.pose.position.y = msg_ptr->pose.position.y;
}
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber obs1_pose_sub =n.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/obstacle1/pose", 100, obs1_pose_set_callback);
